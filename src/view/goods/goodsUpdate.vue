					...
					<el-form ref="dataForm" :rules="rules"  :model="dataForm" label-width="150px">
						...
						<el-form-item label="宣传画廊"  >
							<template>
								...
								<!-- <el-dialog :visible.sync="dataForm.master.length">
								  <img width="100%" :src="item" v-for="item in dataForm.master" alt="">
								</el-dialog> -->
							</template>
						</el-form-item>
						...
						<el-form-item label="商品详情"  >
							 ...
							 <editor :content="$store.state.app.editorContent"></editor>
						</el-form-item>
					</el-form>
				</div>
			</div>
			...
			<div class="box-card">
				<div class="el-card__body">
				    ...
				    <el-dialog title="设置商品参数" :visible.sync="dialogParameterFormVisible">
						<el-form ref='parameterForm' :model="parameterForm" style='width: 400px; margin-left: 50px; ' label-width="100px" label-position="left" >
							<!-- <el-form-item label-width="0" prop="parameter_id">
								<el-input v-model="parameterForm.parameter_id" type='hidden'></el-input>
							</el-form-item> -->
						<!-- 	<el-input v-model="parameterForm.parameter_id" type='hidden'></el-input> -->
							<el-form-item label="商品参数名称" prop="name" style='text-align: left;' >
								<template >
						...
						<div slot="footer" class="dialog-footer">
							<el-button @click="dialogParameterFormVisible = false">取 消</el-button>
							<el-button type="primary" @click="parameterRefer('parameterForm')">确 定</el-button>
						</div>
					</el-dialog>
				</div>
			</div>
			<el-button @click="handleCancel">取消</el-button>
	  		<el-button type="primary" @click="handlePutaway">更新商品</el-button>
	</div>
</template>
<script>
	import Editor from "@/components/editor"
	import Path from "@/api/path"
	import {classList,brandList,goodsMsg,updateData}	from '@/api/goods'
	export default {
		naem: 'GoodsUpdate',
		data (){
			return {
		...
		methods: {
			//商品详情
			updataInfo() {
				var data = { product_id:this.$route.query.id}
				goodsMsg(data).then(function(res){
					console.log(res)
					if(res.data.code == 0){
						this.dataForm = res.data.data
						this.MasterImg = this.masterPath(res.data.data.master)
						this.dynamicTags = res.data.data.antistop?res.data.data.antistop.split(','):[	]
						this.selectedOptions.push(res.data.data.Cate_id)
						this.selectedOptions.push(res.data.data.Sort_id)
						this.$store.state.app.editorContent = res.data.data.contents.contents
						this.$store.state.app.editorContentPath = res.data.data.contents_img.map(function(v,i){
							return v.imgPath
						})
						// console.log(this.$store.state.app.editorContentPath)
						//属性初始化
						this.tableSpeciData = [];
						res.data.data.names.map(function(v,i){							
							v.value.map(function(val,k){
								var obj = {}
								obj['name'] = v.name
								obj['name_id'] = v.name_id
								obj['value'] = val.value
								obj['value_id'] = val.value_id
								obj['img'] = val.img
								this.tableSpeciData.push(obj)
							}.bind(this))			
						}.bind(this))
						this.tableSkuData = res.data.data.sku
						this.tableParameterData = res.data.data.parameter
					}
				}.bind(this))
			},
			//宣传图路径改变
			masterPath(arr) {
				...
				arr.map(function(v,i){
					var obj = { url: v.path_img};
					array.push(obj)
					this.dataForm.master.push(v.path_img)
				}.bind(this))
				return array;
			},
			//删除宣传图
			handleRemove(file, fileList) {
				// 存在两种情况
		        // 1. 如果所删除图片是刚刚上传的图片，那么图片地址是file.response..data[0]
			...
			uploadMasterUrl (res, file, fileLis){
				...
				console.log(this.dataForm.master)
			},
			//删除关键词
			handleClose(tag) {
			...
			speciRefer (){
				...
				console.log(this.tableSpeciData)
			...
			parameterRefer (){
				...
				this.parameterForm = {name:'',value:'',parameter_id:''}
			},
			//商品参数删除
			parameterDel (index){
			...
			//更新商品
			handlePutaway() {
				this.dataForm.master = this.dataForm.master.length?this.dataForm.master:''
				...
				this.$refs['dataForm'].validate((valid) => {
					if (valid) {
						updateData(this.dataForm).then(function(res){
							console.log(res)
							if(res.data.code == 0){
								this.$notify({
					              title: '成功',
